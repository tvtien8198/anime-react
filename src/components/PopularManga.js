...
export default function PopularManga({ mangaList }) {
    return (
        <section id="po-manga" className="lg:h-screen py-12 flex flex-col justify-center">
            <h1 className="mb-8 flex justify-between text-gray-800 dark:text-gray-500">
                <div className="flex items-center">
                    <div className="w-1 mr-2 bg-red-500 h-8"></div>
                    <p className="font-main uppercase sm:text-base lg:text-2xl tracking-wider">Popular Manga</p>
                </div>
            ...
            {
                mangaList.map((manga, id) => 
                <div key={id} className="relative cursor-pointer flex items-center flex-col justify-center">
                    <Link to={`/manga/${manga.mal_id}`} >
                        <img className="shadow-2xl rounded-md md:w-60 sm:w-full w-full md:h-xxl sm:h-full object-cover lazyload blur-up" src={manga.image_url} data-src={manga.image_url} alt={manga.title} />
                    </Link>   
                    <div className="transition duration-300 ease-in-out text-white w-full h-full">   
                        ...
                        <Link to={`/manga/${manga.mal_id}`} >     
                            <span className="lg:text-lg sm:text-sm font-main text-gray-800 dark:text-gray-500">{manga.title}</span>
                        </Link>
                    </div>

...
export default function AnimeDetail() {
    ...
    let videoCode;
    if (trailer_url) {
        videoCode = trailer_url.split('/embed/')[1].split('?')[0];
     }
    ...
    return (
            <section key={mal_id} className="pb-12">
                ...
                <div className="w-full sm:h-ful md:h-full font-main">
                    <div className="transform md:pt-10 sm:pt-4 md:px-10 lg:px-20 sm:px-4 h-ful w-full flex sm:flex-col md:flex-col lg:flex-row justify-center overflow-auto">
                        ...
                        <div className="lg:w-2/3 md:w-full sm:w-full py-4 text-gray-500">
                            ...
                            <p className="text-sm mb-2 tracking-wider">
                                <span className="font-bold mr-2 text-gray-800 dark:text-gray-500">Episodes:</span>{episodes}  -  
                                <span className="font-bold ml-2 text-gray-800 dark:text-gray-500">Duration:</span>  {duration}
                            </p>
                            <p className="text-sm mb-2 tracking-wider"><span className="font-bold mr-1 text-gray-800 dark:text-gray-500">Aired:</span> {aired && aired.string}</p>
                    
                            <p className="text-sm mb-2 tracking-wider" >
                                <span className="font-bold text-gray-800 dark:text-gray-500 mr-1">Studios:</span>
                                {producers && producers.map((prod, id) => <span key={id}>{prod.name}, </span>)}
                            </p>
                            <p className="mb-4 text-sm font-bold flex items-center">
                                <span className="mr-2 text-gray-800 dark:text-gray-500">Trailer: </span>
                                <button onClick={openModal} className="px-2 py-1 rounded-lg shadow-md bg-blue-500 hover:bg-blue-400 transition duration-300 text-white flex ">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" />
                                    </svg><span className="ml-2">Play now</span>
                                </button>
                            </p>
                        </div>
                    </div>
                    <div className="w-full lg:px-24 md:px-10 sm:px-4 lg:pt-10 sm:pt-4">
                    ...
                    <ModalVideo channel='youtube' isOpen={isOpen} videoId={videoCode} onClose={() => setOpen(false)} />
                </div>
            </section>

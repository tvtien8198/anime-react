...
import { GetListData} from './../../apis/index';
...
export default function AnimeViewAll() {
    ...
    return (
        <section className="py-12 flex flex-col justify-center">   
            <Link to="/" className="flex text-gray-800 text-lg items-center mb-4">
            ...
            <div className="mb-8">
                <p className="font-main uppercase text-4xl text-gray-800 dark:text-gray-500 tracking-wider text-center">Anime</p>
            </div>
            <div className="grid lg:grid-cols-4 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {
                animeList.slice(0, count).map((anime, id) =>

                <div key={id} className="relative cursor-pointer flex items-center flex-col justify-center">
                    <Link to={`/anime/${anime.mal_id}`} >
                        <img className="shadow-xl rounded-md lg:w-60 sm:w-full sm:h-full lg:h-xxl object-cover lazyload blur-up" src={anime.image_url} data-src={anime.image_url} alt={anime.title} />
                    </Link>   
                    <div className="transition duration-300 ease-in-out text-white w-full h-full">   
                        ...
                        <Link to={`/anime/${anime.mal_id}`} >     
                            <span className="text-lg font-main text-gray-800 dark:text-gray-500">{anime.title}</span>
                        </Link>
                    </div>
